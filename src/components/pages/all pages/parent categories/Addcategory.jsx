...
export default function AddCategory() {
  ...
  let handleSubmit = (e) => {
    ...
    axios[getOrPost](`${baseUrl}category/${addOrUpdate}`, myData)
      .then((res) => {
        if (res.data._status) {
          successToast(res.data._message)
          setTimeout(() => {
            navigate('/parent-categories/view')
          }, 2000)
        }
        else {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`${baseUrl}category/single/${id}`)
        .then((res) => {
          console.log(res.data)
          if (res.data._status) {
            setCategoryName(res.data._data.name)
  ...
  return (
    <div className="border border-blue-300 rounded-lg bg-white">
      ...
      <div className="px-6 py-4 border-b bg-slate-50 rounded-t-lg">
        <h2 className="text-xl font-semibold">{id ? "Update" : "Add"} Category</h2>
      </div>
