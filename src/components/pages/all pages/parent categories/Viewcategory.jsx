...
export default function Viewcategory() {
  ...
  let getCategoryData = () => {
    axios.get(`${baseUrl}category/view`)
      .then(res => {
        if (res.data._status) {
  ...
  let allCheck = (e) => {
    if (e.target.checked) {
      setChecked(categoryData.map((item) => item._id))
    }
    else {
      setChecked([])
    }
  ...
  let handleDelete = () => {
    ...
    axios.post(`${baseUrl}category/delete`, { ids: checked })
      .then(res => {
        if (res.data._status) {
          Swal.fire({
            title: "Are you sure?",
  ...
  let changeStatus = () => {
    ...
    axios.post(`${baseUrl}category/change-status`, { ids: checked })
      .then(res => {
        if (res.data._status) {
          Swal.fire({
            title: "Are you sure?",
            ...
            <thead className="bg-[#374151] text-gray-400">
              <tr>
                <th className="p-4"><input onChange={allCheck} checked={
                  ...
                  checked.length === categoryData.length
                } type="checkbox" /></th>
                ...
                >
                  ...
                  <td className="p-4">
                    <button className="bg-blue-600 p-3 rounded-full text-white">
                      <Link to={`/parent-categories/add/${user._id}`}>
                        <FaEdit />
                      </Link>
                    </button>
                  </td>
