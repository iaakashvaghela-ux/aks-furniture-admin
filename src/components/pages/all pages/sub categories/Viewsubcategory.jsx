...
export default function Viewsubcategory() {
  ...
  let getsubCategoryData = () => {
    axios.get(`${baseUrl}sub-category/view`)
      .then(res => {
        if (res.data._status) {
  ...
  let allCheck = (e) => {
    if (e.target.checked) {
      setChecked(subCategoryData.map((item) => item._id))
    }
    else {
      setChecked([])
    }
  ...
  let handleDelete = () => {
    ...
    axios.post(`${baseUrl}sub-category/delete`, { ids: checked })
      .then(res => {
        if (res.data._status) {
          Swal.fire({
            title: "Are you sure?",
  ...
  let changeStatus = () => {
    ...
    axios.post(`${baseUrl}sub-category/change-status`, { ids: checked })
      .then(res => {
        if (res.data._status) {
          Swal.fire({
            title: "Are you sure?",
      ...
      <div className="w-full max-w-[1220px] space-y-6">
        ...
        <div className="bg-[#1f2937] rounded-lg overflow-hidden shadow-lg">
          ...
          <div className="flex justify-between items-center p-4 bg-gray-100">
            <h2 className="text-xl font-semibold text-gray-800">View sub category</h2>
                ...
                >
                  ...
                  <td className="p-4">
                    <button className="bg-blue-600 p-3 rounded-full text-white">
                      <Link to={`/sub-categories/add/${user._id}`}>
                        <FaEdit />
                      </Link>
                    </button>
                  </td>
